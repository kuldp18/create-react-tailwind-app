  ...
  .action(async (projectName, options) => {
    ...
    try {
      if (fs.existsSync(projectPath)) {
        spinner.fail(
          `A folder named ${chalk.yellow(finalProjectName)} already exists.`
        );
        process.exit(1);
      }
...
async function createViteProject(
  ...
  try {
    ...
    if (packageManager === "npm") {
      execSync(
        `npm create vite@latest ${projectName} -- --template ${reactTemplate}`,
        {
...
export default config;`;
    ...
    const mainFile =
      templateType === "typescript" ? "src/main.tsx" : "src/main.jsx";
    ...
    if (!mainContent.includes("./index.css")) {
      if (mainContent.includes("import React from 'react'")) {
        updatedMainContent = mainContent.replace(
    ...
    const exampleComponent = `${
      templateType === "typescript" ? 'import React from "react"\n\n' : ""
    }const Example = () => {
  return (
    <div className="flex flex-col items-center justify-center bg-gradient-to-r from-purple-500 to-pink-500 rounded-2xl shadow-2xl p-16 gap-4 shadow-purple-400">
      ...
      <p className="text-white text-xl">Edit <code className="bg-white/20 px-1 py-0.5 rounded animate-pulse">src/components/Example.${
        templateType === "typescript" ? "tsx" : "jsx"
...
export default Example`;
    ...
    const appFile =
      templateType === "typescript" ? "src/App.tsx" : "src/App.jsx";
...
export default App`;
    ...
    // Ensure latest React and ReactDOM are installed
    console.log(
      chalk.cyan(`\nUpdating React and ReactDOM to latest version...`)
    );

    const reactInstallCmd =
      packageManager === "npm"
